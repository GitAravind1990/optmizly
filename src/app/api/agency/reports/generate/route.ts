...
// output is never treated as HTML, then convert its habitual markdown **bold**
// (never instructed, but consistently produced) into real <strong> tags instead of
// leaving literal asterisks visible in a client-facing document.
function escapeHtml(s: string): string {
  return s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;')
}
function renderSummary(s: string): string {
...
function generateReportHtml(data: {
  ...
  // This template gets rendered on a public, unauthenticated page via
  // dangerouslySetInnerHTML — every one of these is agency-user-supplied at client
  // creation/update time (name, website, brandColor) or from the user's own tracked-
  // keyword list, so none of it can be trusted as pre-sanitized HTML/attribute-safe text.
  const safeName = escapeHtml(client.name)
  const safeWebsite = escapeHtml(client.website)
  const brand = escapeHtml(client.brandColor ?? '#6366f1')
  ...
  const topKwRows = topKeywords.length
    ? topKeywords
        ...
        <tr style="border-bottom:1px solid #f1f5f9">
          <td style="padding:10px 16px;font-size:14px;color:#334155">${escapeHtml(keyword)}</td>
...
<title>SEO Report — ${safeName} — ${monthName} ${year}</title>
</head>
  ...
  <div style="max-width:800px;margin:40px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 32px rgba(0,0,0,.08)">
    ...
    <div style="background:${brand};padding:40px;color:#fff">
      <div style="display:flex;justify-content:space-between;align-items:flex-start">
        <div>
          <div style="font-size:28px;font-weight:800;margin-bottom:4px">${safeName}</div>
          <div style="font-size:14px;opacity:.85">${safeWebsite}</div>
        </div>
    ...
    <div style="padding:40px">
      ...
      <p style="font-size:14px;line-height:1.7;color:#475569;background:#f8fafc;padding:20px;border-radius:10px;border-left:4px solid ${brand};margin:0 0 32px">${renderSummary(aiSummary)}</p>
    ...
    <div style="background:#f8fafc;border-top:1px solid #e2e8f0;padding:24px 40px;display:flex;justify-content:space-between;align-items:center">
      ...
      <div style="font-size:13px;color:#94a3b8">Confidential — ${safeName} — ${monthName} ${year}</div>
    </div>
  </div>
</body>
</html>`
